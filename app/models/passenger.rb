class Passenger < ActiveRecord::Base
  ...
  def self.insert(params)
    ...
    query3 = "UPDATE users SET credit = credit - #{p} WHERE email = '#{params[:email]}';"
    ActiveRecord::Base.connection.execute(query3)


    query1 = "INSERT INTO passengers (email, start_time, car_plate, onboard) VALUES('#{params[:email]}', '#{params[:start_time]}', '#{params[:car_plate]}', '#{params[:onboard]}');"
    ActiveRecord::Base.connection.execute(query1)


    query4 = "UPDATE users u SET credit = credit + #{p} "\
